...
import { buildShape, lintErrors, compile } from "../src/compiler/index.ts";
import { parseSpecText } from "../src/compiler/parse.ts";
import { SESSION_STATE_KIND } from "../src/session-state.ts";

// Continuation boundaries stay keyless: reserved-arg lint rejects workflow
// collisions, while weak-evidence validation, reserved-key injection, and
// malformed-prior handling run as real CLI subprocesses on command fixtures.
...
// --- 1. reserved-arg lint (compiler, no model) -------------------------------
...
// --- 2. weak-evidence --stdin gate (real subprocess, model-free) -------------
...
// --- 3. reserved-key injection reaches the workflow input (subprocess) --------
...
// --- 4. malformed prior on -t is a LOUD exit 1 (subprocess) ------------------

...
Navi's managed DeepSeek Flash baseline is thinking enabled at max effort. A
step-local setting overrides that baseline; run-level `--thinking` and
`--reasoning-effort` flags override it for one invocation.
When a step sets `thinking: disabled` without an effort, Navi resolves its
effective effort to `low` so the inherited max value cannot survive. An effort
explicitly authored in the same step remains authoritative.

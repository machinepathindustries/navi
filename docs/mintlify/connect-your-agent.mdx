...
The interop skill teaches boundary-based use: the coding agent owns routine,
reversible implementation; Navi is optional at a consequential decision seam
and used once for a coherent delivery check. It does not require approval for
every file, phase, repair, or command.

Verify the project-local launcher:

```bash
./.agents/bin/navi --version
./.agents/bin/navi catalog
./.agents/bin/navi help brainstorm
```

The catalog is the cold-start map. Begin ordinary, reversible work directly
when the task is already clear. Use one Brainstorm run only when materially
different viable ideas remain and choosing among them would change the outcome;
the flow help prints its exact JSON event contract. Navi then returns control to
the calling agent instead of supervising each implementation step.
